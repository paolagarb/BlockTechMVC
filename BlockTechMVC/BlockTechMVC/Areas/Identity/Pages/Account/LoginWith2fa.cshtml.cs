...
namespace BlockTechMVC.Areas.Identity.Pages.Account
{
    ...
    public class LoginWith2faModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (user == null)
            {
                throw new InvalidOperationException($"Não é possível carregar a autenticação de dois fatores.");
            }
        ...
        public async Task<IActionResult> OnPostAsync(bool rememberMe, string returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("Usuário'{UserId}' conectado com 2fa.", user.Id);
                return LocalRedirect(returnUrl);
            }
            else if (result.IsLockedOut)
            {
                _logger.LogWarning("Usuário'{UserId}' bloqueado.", user.Id);
                return RedirectToPage("./Lockout");
            }
            else
            {
                _logger.LogWarning("Código de autenticação inválido inserido para o usuário '{UserId}'.", user.Id);
